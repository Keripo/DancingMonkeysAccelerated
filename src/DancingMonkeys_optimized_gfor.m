...
function DancingMonkeys_optimized_gfor( varargin )
...
%     else
	...
	timeConvert = tic;
	IntervalFrequency_g  = gdouble(IntervalFrequency);
	MinimumInterval_g    = gdouble(MinimumInterval);
	HalfGapWindowSize_g  = gdouble(HalfGapWindowSize);
	Beats_g              = gdouble(Beats);
	BeatStrengths_g      = gdouble(BeatStrengths);
	GapWindow_g          = gdouble(GapWindow);
	NumBeats_g           = gdouble(NumBeats);
	NumBeatsM_g          = gdouble(NumBeats - 1);
	SizeBeats_g          = gdouble(size( Beats,1 ));
	
	Interval_g           = gzeros(kMax, 1);
	SortedGaps_g         = gzeros(NumBeats, kMax); 
	SortedIndex_g        = gzeros(NumBeats, kMax);
	GapsFiltered_g       = gzeros(NumBeats, kMax);
	GapsConfidence_g     = gzeros(NumBeats, kMax);
	displog( ImportantMsg, LFN, sprintf( '>>> timeConvert = %f', toc(timeConvert) ) );
	
	timeSort = tic;
	gfor k = 1:kMax
		i = (k - 1) * IntervalFrequency_g + MinimumInterval_g;
		Gaps = mod(Beats_g, i);
		ExtraGaps = Gaps + i;
		FullGaps = [ Gaps ExtraGaps ]';
        FullGaps = FullGaps(:);
		[ SortedGaps_g(:,k) SortedIndex_g(:,k) ] = sort( FullGaps );
		Interval_g(k) = i;
	gend
	displog( ImportantMsg, LFN, sprintf( '>>> timeSort = %f', toc(timeSort) ) );
    ...
    gfor k = 1:kMax
        ...
        for ct1 = 1 : NumBeats
            ...
            GapsFiltered_g(ct1, k) = Area;

        end
	gend
	displog( ImportantMsg, LFN, sprintf( '>>> timeFilter = %f', toc(timeFilter) ) );
	...
	gfor k = 1:kMax
        ...
        for ct1 = 1 : NumBeats -1 
            ...
            % pointless. TODO?
            OffBeatValue = ( GapsFiltered_g(Pos, k) + GapsFiltered_g(Pos + 1, k) ) / 2;

            GapsConfidence_g(ct1, k) = GapsFiltered_g(ct1, k) + ( OffBeatValue / 2 );        
			
        end
	gend
	displog( ImportantMsg, LFN, sprintf( '>>> timeConfidence = %f', toc(timeConfidence) ) );
	
	timeMax = tic;
	SortedGaps = double(SortedGaps_g);
	GapsConfidence = double(GapsConfidence_g);
	for k = 1:kMax
		i = (k - 1) * IntervalFrequency + 1;
		MaxGapsConfidence = max(GapsConfidence(:, k));
        GapPeaks = SortedGaps(GapsConfidence(:,k) == MaxGapsConfidence, k);
		
        IntervalFitness(i) = MaxGapsConfidence;
        IntervalGap(i) = GapPeaks(1);
    end
	displog( ImportantMsg, LFN, sprintf( '>>> timeMax = %f', toc(timeMax) ) );
	...
	GapsFiltered_g = gzeros( NumBeats, 1 );
	GapsConfidence_g = gzeros( NumBeats, 1 );
	IntervalFitness_g = gdouble(IntervalFitness);
    gfor k = 1:kMax
        ...
        if (IntervalFitness_g(k) == -1)
            ...
            FullGaps = [ Gaps ExtraGaps ]';
            FullGaps = FullGaps(:);
            [ SortedGaps_g SortedIndex_g ] = sort( FullGaps );

            for ct1 = 1 : NumBeats
                Area = 0;

                Centre = SortedGaps_g( ct1 );

                Pos = ct1;
                PosVal = SortedGaps_g( Pos );
                while ( PosVal > Centre - HalfGapWindowSize_g ) & (Pos > 1)
            ...
            for ct1 = 1 : NumBeats -1 
                ...
                OffbeatPos = SortedGaps_g( ct1 ) + round(i / 2);
                ...
                % gaps. Might give some improvement to accuracy, but most probably
                % pointless.
                OffBeatValue = ( GapsFiltered_g( Pos ) + GapsFiltered_g( Pos + 1 ) ) / 2;

                GapsConfidence_g( ct1 ) = GapsFiltered_g( ct1 ) + ( OffBeatValue * 0.5 );        
            end
		end
	gend
