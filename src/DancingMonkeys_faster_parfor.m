...
%   modified by Philip Peng, version 1.07 on, 2012

function DancingMonkeys_faster_parfor( varargin )
...
if ( ~ exist( RootOutputDirectory, 'dir' ) )
    [ RootOutputParentDirectory, RODirectory, Temp ] = fileparts( RootOutputDirectory );
...
%    [ BpmPath, BpmFilename , BpmFileExt ] = fileparts( BpmFullFilename );
...
%     else
    ...
    parfor k = 1:kMax
        ...
        for ct1 = 1 : NumBeats
            ...
            while ( PosVal > Centre - HalfGapWindowSize ) & ( Pos > 1 )
                ...
                xPos = xPos - (xPos > SizeBeats) * SizeBeats;
		...
		MaxGapsConfidence = max( GapsConfidence );
        GapPeaks = SortedGaps( find( GapsConfidence == MaxGapsConfidence ) );

        IntervalFitnessP(k) = MaxGapsConfidence;
    ...
    displog( ProgressMsg, LFN, sprintf( 'BPM testing time is %f seconds', toc(timeTest) ) );
    timeFitness = tic;
    ...
    displog( ProgressMsg, LFN, sprintf( 'BPM fitness checking time is %f seconds', toc(timeFitness) ) );
...
% Close pool for parallel computing
matlabpool close;