...
%     else
    ...
    %for i = MinimumInterval : IntervalFrequency : MaximumInterval
		...
		disp(sprintf('timeBeatsEtc_g = %f', toc(timeBeatsEtc_g)))
		
		timeBeatsEtc_j = tic;
		Beats_j = double(Beats);
		BeatStrengths_j = double(BeatStrengths);
		GapWindow_j = double(GapWindow);
		HalfGapWindowSize_j = double(HalfGapWindowSize);
		BeatsSize_j = double(size( Beats_g,1 ));
		One_j = gones(1, 1, 'double');
		disp(sprintf('timeBeatsEtc_j = %f', toc(timeBeatsEtc_j)))
		...
		timeCalcI_j = tic;
        l = double(i);
        disp(sprintf('timeCalcI_j = %f',toc(timeCalcI_j)))
		...
		timeGaps_j = tic;
		Gaps_j = mod( Beats_j, l );
        ExtraGaps_j = Gaps_j + l;
		disp(sprintf('timeGaps_j = %f', toc(timeGaps_j)))
		...
		timeFullGaps_j = tic;
        FullGaps_j = [ Gaps_j ExtraGaps_j ]';
        FullGaps_j = FullGaps_j(:);
		disp(sprintf('timeFullGaps_j = %f', toc(timeFullGaps_j)))
		...
		timeSortFullGaps_j = tic;
        [ SortedGaps_j SortedIndex_j ] = sort( FullGaps_j );
		disp(sprintf('timeSortFullGaps_j = %f', toc(timeSortFullGaps_j)))
		...
		timeGapsFiltered_j = tic;
        GapsFiltered_j = gzeros(NumBeats, 1, 'double');
		disp(sprintf('timeGapsFiltered_j  = %f', toc(timeGapsFiltered_j)))
		...
		timeAssignment_j = tic;
		Area_j = gzeros(1, 1, 'double');
		ct1_j = gones(1, 1, 'double');
		Pos_j = ct1_j;
		disp(sprintf('timeAssignment_j = %f', toc(timeAssignment_j)))
		...
		timeLookups_j = tic;
		Centre_j = SortedGaps_j( ct1_j );
		PosVal_j = SortedGaps_j( Pos_j );
		xPos_j = SortedIndex_j( Pos_j );
		StrengthVal_j = BeatStrengths_j( Centre_j );
		GapWindowVal_j = GapWindow_j( PosVal_j );
		disp(sprintf('timeLookups_j  = %f', toc(timeLookups_j)))
		...
		timeComparison_j = tic;
		whileCheck_j = PosVal_j > Centre_j - HalfGapWindowSize_j;
		ifCheck1_j = Pos_j <= One_j;
		ifCheck2_j = xPos_j > BeatsSize_j;
		disp(sprintf('timeComparison_j  = %f', toc(timeComparison_j)))
		...
		timeCalculations_j = tic;
		xPos_j = xPos_j - BeatsSize_j;
		GapWindowIndex_j = PosVal_j - (Centre_j - HalfGapWindowSize_j);
		Area_j = Area_j + ( StrengthVal_j * GapWindowVal_j );
        Pos_j = Pos_j - One_j;
		disp(sprintf('timeCalculations_j = %f', toc(timeCalculations_j)))
		...
		timeAssignBack_j = tic;
		GapsFiltered_j( ct1_j ) = Area_j;
		disp(sprintf('timeAssignBack_j = %f', toc(timeAssignBack_j)))
