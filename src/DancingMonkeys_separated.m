...
%     else
	...
	%timeConvert = tic;
	IntervalFrequency_g  = double(IntervalFrequency);
	MinimumInterval_g    = double(MinimumInterval);
	HalfGapWindowSize_g  = double(HalfGapWindowSize);
	Beats_g              = double(Beats);
	BeatStrengths_g      = double(BeatStrengths);
	GapWindow_g          = double(GapWindow);
	NumBeats_g           = double(NumBeats);
	NumBeatsM_g          = double(NumBeats - 1);
	SizeBeats_g          = double(size( Beats,1 ));
	
    IntervalFitness_g    = zeros(kMax, 1);
    IntervalGap_g        = zeros(kMax, 1);
	Interval_g           = zeros(kMax, 1);
	SortedGaps_g         = zeros(NumBeats, kMax); 
	SortedIndex_g        = zeros(NumBeats, kMax);
	GapsFiltered_g       = zeros(NumBeats, kMax);
	GapsConfidence_g     = zeros(NumBeats, kMax);
    ...
    for k = 1:kMax
        ...
        for ct1 = 1 : NumBeats
            ...
            while ( PosVal > Centre - HalfGapWindowSize_g )
	...
	%timeMax = tic;
	for k = 1:kMax
        GapPeaks = SortedGaps_g(GapsConfidence_g(:,k) == max(GapsConfidence_g(:, k)), k);
		
        IntervalFitness_g(k) = max(GapsConfidence_g(:,k));
        IntervalGap_g(k) = GapPeaks(1);
    end
    ...
    for k = 1:kMax
        ...
        IntervalFitness(i) = double(IntervalFitness_g(k));
        IntervalGap(i) = double(IntervalGap_g(k));
    end
