...
%     else
	...
	%{
	HalfGapWindowSize_g = HalfGapWindowSize;
	BeatStrengths_g = BeatStrengths;
	GapWindow_g = GapWindow;
	NumBeats_g = NumBeats;
	%}
	%%{
	...
	% Constant
	sizeBeats = size(Beats, 1);
	%sizeBeats_g = sizeBeats;
	sizeBeats_g = gdouble(sizeBeats);
	%one_g = ones(1, 1);
	one_g = gones(1, 1);
	%zero_g = zeros(1, 1);
	zero_g = gzeros(1, 1);
	
	loopSize = length(1 : NumBeats);
	%{
	Area_v = zeros(loopSize, 1);
	Centre_v = zeros(loopSize, 1);
	Pos_v = zeros(loopSize, 1);
	PosVal_v = zeros(loopSize, 1);
	xPos_v = zeros(loopSize, 1);
	sizeCheck_v = zeros(loopSize, 1);
	%}
	%%{
	Area_v = gzeros(loopSize, 1);
	Centre_v = gzeros(loopSize, 1);
	Pos_v = gzeros(loopSize, 1);
	PosVal_v = gzeros(loopSize, 1);
	xPos_v = gzeros(loopSize, 1);
	sizeCheck_v = gzeros(loopSize, 1);
	%%}
    ...
    %for i = MinimumInterval : IntervalFrequency : MaximumInterval
		...
		% Casting
		%{
		GapsFiltered_g = GapsFiltered;
		SortedGaps_g = SortedGaps;
		SortedIndex_g = SortedIndex;
		%}
		%%{
		GapsFiltered_g = gdouble(GapsFiltered);
		SortedGaps_g = gdouble(SortedGaps);
		...
		gfor n = 1 : NumBeats
            Area_v(n) = zero_g;
            Centre_v(n) = SortedGaps_g(n);
            Pos_v(n) = n;
            PosVal_v(n) = SortedGaps_g( n );
            while ( PosVal_v(n) > Centre_v(n) - HalfGapWindowSize_g ) & ( Pos_v(n) > one_g )
                xPos_v(n) = SortedIndex_g( Pos_v(n) );
                sizeCheck_v(n) = xPos_v(n) > sizeBeats_g;
                xPos_v(n) = xPos_v(n) - sizeCheck_v(n) * sizeBeats_g;
                Area_v(n) = Area_v(n) + ( BeatStrengths_g( xPos_v(n) ) * GapWindow_g( PosVal_v(n) - (Centre_v(n) - HalfGapWindowSize_g) ) );
                Pos_v(n) = Pos_v(n) - one_g;
                PosVal_v(n) = SortedGaps_g( Pos_v(n) );
            end

            Pos_v(n) = n;
            PosVal_v(n) = SortedGaps_g( n );
            while ( PosVal_v(n) <= Centre_v(n) + HalfGapWindowSize_g ) & ( Pos_v(n) < NumBeats_g )
                xPos_v(n) = SortedIndex_g( Pos_v(n) );
                sizeCheck_v(n) = xPos_v(n) > sizeBeats_g;
                xPos_v(n) = xPos_v(n) - sizeCheck_v(n) * sizeBeats_g;
                Area_v(n) = Area_v(n) + ( BeatStrengths_g( xPos_v(n) ) * GapWindow_g( PosVal_v(n) - (Centre_v(n) - HalfGapWindowSize_g) ) );
		...
		% Casting
		%GapsConfidence_g = GapsConfidence;
		GapsConfidence_g = gdouble(GapsConfidence);
		
		NumBeatsLess = NumBeats - 1;
		NumBeatsLess_g = NumBeats_g - one_g;
		
        %for n = 1 : NumBeatsLess
		gfor n = 1 : NumBeatsLess
            OffbeatPos = SortedGaps_g( n ) + round(i / 2);

            % We know the position of where an offbeat gap value would be but
            % we need to work out its index in the SortedGaps array
            Pos_v(n) = n;
            PosVal_v(n) = SortedGaps_g( Pos_v(n) );
            while ( PosVal_v(n) < OffbeatPos ) & (Pos_v(n) < NumBeatsLess_g)
                Pos_v(n) = Pos_v(n) + one_g;
                PosVal_v(n) = SortedGaps_g( Pos_v(n) );
            end
            ...
            % pointless. TODO?
            OffBeatValue = ( GapsFiltered_g( Pos_v(n) ) + GapsFiltered_g( Pos_v(n) + one_g ) ) / 2;

            GapsConfidence_g( n ) = GapsFiltered_g( n ) + ( OffBeatValue * 0.5 );        
        %end
		gend
    ...
    for k = 1:kMax
        ...
        IntervalFitness(i) = double(IntervalFitnessP(k));
        IntervalGap(i) = double(IntervalGapP(k));
    end
	displog( ImportantMsg, LFN, sprintf( '>>> timeTestCopy = %f', toc(timeTestCopy) ) );
