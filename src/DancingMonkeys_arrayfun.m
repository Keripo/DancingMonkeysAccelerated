...
%     else
    ...
    %for k = 1:kMax
	%	[ IntervalFitnessP(k), IntervalGapP(k) ] = arrayfun_BpmTest(k);
    %end
	
	% Using arrayfun
	k = 1:kMax;
	[ IntervalFitnessP, IntervalGapP ] = arrayfun(@arrayfun_BpmTest, k);
	...
	% Using normal for loop
    %for k = 1:kMax
	%	[ IntervalFitness(k), IntervalGap(k) ] = arrayfun_FitTest(k, IntervalFitness(k), IntervalGap(k));
    %end
	
	% Using arrayfun
	k = 1:kMax;
	[ IntervalFitnessF, IntervalGapF ] = arrayfun(@arrayfun_FitTest, k);
    
    % Copy over values
    for k = 1:kMax
        IntervalFitness(k) = IntervalFitnessF(k);
        IntervalGap(k) = IntervalGapF(k);
    end
